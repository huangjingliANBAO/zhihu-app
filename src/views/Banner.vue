	...
	<div>
		<div class="nav-bar shadow mb-2 fill-white">
			<transition name="nav-bar">
				<div class="container row" v-if="isUp">
					...
					<ul class="fruid-col-2 row justify-content-around">
						<li>
							<router-link to="/home" class="default-nav">首页</router-link>
						</li>
						<li>
							<router-link to="/explore" class="default-nav">发现</router-link>
						</li>
						<li>
							<router-link to="/question/waiting" class="default-nav">等你来答</router-link>
						</li>
					</ul>
					...
					<div class="fruid-col-5 d-flex align-items-center pl-4 search-bar">
						...
						<transition name="fade">
							<div class="search-container border" v-if="popUp">
								...
								<a href="" class="grey border-bottom d-block">全国确诊24363例</a>
								<a href="" class="grey border-bottom d-block">湖北红会3领导被问责</a>
								<a href="" class="grey border-bottom d-block">石正丽</a>
								<a href="" class="grey border-bottom d-block">武汉病毒研究所</a>
								<a href="" class="grey border-bottom d-block">李兰娟团队发布重大研究成果</a>
								<a href="" class="border-bottom d-block">男子武汉返乡却谎称菲律宾回来</a>
								<a href="" class="grey border-bottom d-block">武汉方舱医院</a>
							</div>
						</transition>
			...
			<transition name="hide-bar">
				<div class="container row justify-content-around  hide-bar" v-if="!isUp">
					<div class="d-flex align-items-center col-1">
...
<script>
	...
	mounted() {	
		...
		// 初始化滚动条为位置为0
		let scrollTop = 0; 
		// 设置一个标识位，即复制一个滚动条位置，但是这个位置获取的时间比 scrollTop慢
		let topValue = this.getScrollTop();
		document.onscroll = function() {
			// 滚动条的位置
			scrollTop = _this.getScrollTop(); 
			if (scrollTop <= topValue) {
				// 当后者滚动条大于前者滚动条时，即认为滚动条向上运动，
				//但是我们设置一个临界值，当大于这个临界值时，即认为是用户有意向上滚动
				//  changeUpDir 这个是 刚好从向下滚动到向上滚动改变方向时的位置
